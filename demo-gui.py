...
# Load the YOLO model
model = YOLO('yolo-models/yolo11m.pt')
...
root = tk.Tk()
root.title("YOLO 11 Video Detection")
...
def update_confidence(value):
    global confidence_threshold, confidence_slider
    confidence_threshold = int(value) / 100

confidence_threshold = 0.5

# Confidence slider
confidence_slider = tk.Scale(control_frame, from_=0, to=100, orient=tk.HORIZONTAL, label="Confidence Threshold", command=update_confidence)
confidence_slider.set(int(confidence_threshold * 10))
...
def toggle_select_deselect():
    ...
    if is_selected:
        for var in label_vars.values():
            var.set(True)
        clear_button.config(text='Deselect All Classes')
    else:
        for var in label_vars.values():
            var.set(False)
        clear_button.config(text='Select All Classes')

clear_button = tk.Button(root, text='Deselect All Classes', command=toggle_select_deselect)
clear_button.pack(pady=10)

# Initialize with entire frame selected
is_entire_frame = True

# Create a button to select the entire frame
def select_entire_frame():
    global polygon_points, is_entire_frame
    if is_entire_frame:
        polygon_points = []
    else:
        polygon_points = [(0, 0), (1080, 0), (1080, 720), (0, 720)]
    is_entire_frame = not is_entire_frame
    ...
    if is_entire_frame:
        select_entire_frame_button.config(text='Select Entire Frame')
    else:
        select_entire_frame_button.config(text='Deselect Entire Frame')

select_entire_frame_button = tk.Button(root, text='Select Entire Frame', command=select_entire_frame)
select_entire_frame_button.pack(pady=10)
